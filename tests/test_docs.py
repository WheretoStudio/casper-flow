...
The README states defaults and option values that config.py and settings.json
own. Docs nobody checks become fiction, so a script checks them.
"""
...
# Working documents, not part of what gets published. The checks below run
# against them when present and skip when not: CI clones only what is shipped.
...
class TestSettingsAreDocumented:
    ...
    def test_documented_default_matches_the_shipped_value(self, readme, shipped, key):
        """The documented default must match settings.json. Rows that state no
        literal default are skipped."""
...
class TestNoContradictions:
    """A value the README calls the default must be the default, and an option it
    offers must exist in the code."""

    # Phrases that assert a default.
    DEFAULT_CLAIMS = [
    ...
    def test_output_script_values_exist_in_the_code(self, readme):
        """A value the README offers but config.py rejects falls back silently."""
    ...
    def test_no_markdown_table_is_split_by_prose(self, repo_root):
        """A paragraph inside a table turns the remaining rows into plain text."""
...
class TestNoStaleProductNames:
    @pytest.mark.parametrize("name", [
        ...
        # LICENSE and installer.iss are read inside the installer itself.
    ...
    def test_old_names_are_gone(self, repo_root, name):
        ...
        def excused(i: int) -> bool:
            # The marker may sit above the line: in installer.iss `;` separates
            # parameters, so a trailing comment there is a syntax error.
...
class TestPlanIsHonest:
    """Measured and published numbers must stay distinguishable in the plan."""
    ...
    def test_every_phase_has_an_acceptance_criterion(self, plan):
        ...
        for phase in phases:
            ...
            # A phase ends at the next phase heading or top-level section, not at
            # the next "### ": a phase may contain its own subsections.
