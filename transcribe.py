...
preload() loads the local model at startup: a cold load of `small` costs seconds
and the first run downloads ~460 MB.
"""
...
_local_model = None
_local_key = None
# What actually loaded, which may not be what was requested - see the fallback in
# _load_local. None until the first load.
...
def transcribe(audio_path: Path, cfg: dict) -> str:
    ...
    # Privacy gate: enforced here, not left to the config being right.
...
# A loadable snapshot needs all of these. An interrupted download leaves the repo
# directory present but empty, so the directory alone proves nothing.
_REQUIRED_FILES = {"config.json", "model.bin", "tokenizer.json"}


def _cached_models() -> list[str]:
    """
    Models already present and loadable, best fallback first.

    A model arrives as a Systran repo (bare size, "base"), a local conversion under
    models/, or any other HuggingFace "owner/name" id. All three count, or a user
    whose only complete model is a local conversion is told there is none.
    """
    ...
    # Known sizes first, cheapest first: a fallback needs to load fast, not be
    # accurate. Local conversions next - already on disk, no network.
...
def resolve_model(name: str) -> str:
    ...
    A name matching a directory under models/ becomes an absolute local path, so a
    converted model is selectable by name like a built-in one; anything else passes
    through as a HuggingFace id. Absolute because the app launches from a shortcut,
    a .bat and a scheduled task, which disagree on the working directory.
    """
    if not name:
        return name
    for base in _model_dirs():
...
def _load_local(cfg: dict):
    ...
    # DEFAULTS, not a second literal: a missing key must load the model the docs,
    # tests and profiles all assume ships.
    ...
    with _model_lock:
        ...
        try:
            ...
            if device != "cpu":
                ...
                # Usually no network on first run; use whatever is downloaded
                # rather than failing every dictation.
                cached = [m for m in _cached_models() if m != model_size]
                if not cached:
                    raise
                # Smallest complete model first: usable beats accurate here.
                fallback = cached[0]
                log.warning(
                ...
                # int8, not the requested compute_type: an unsupported one is a
                # reason we got here, and failing again inside this except block
                # breaks every dictation. int8 works wherever CTranslate2 does.
                model = build(fallback, "cpu", "int8")
                actual = fallback

        _local_model = model
        # Keyed on the REQUESTED name: keying on the fallback would miss the cache
        # every call and retry the load that just failed.
        _local_key = key
        # Since the key is the requested name, this is the only way callers can see
        # a substitution happened. doctor.py reports it.
...
def _load_preview(cfg: dict):
    """
    Load the small model used for live captions. Worth the extra memory: previews
    run repeatedly while you speak, and the main model makes each pass cost as much
...
def transcribe_array(audio, cfg: dict) -> str:
    ...
    Skips the WAV round-trip and is tuned for speed: small model, no VAD, greedy, no
    timestamps. Display only - the final pass re-transcribes everything.
    """
    ...
    # Previews share the prompt, so they can echo it too.
...
def leaked_prompt(text: str, prompt: str) -> bool:
    ...
    Whisper conditions on initial_prompt as if it had just transcribed it, so on
    unclear audio it continues the prompt and we paste words nobody said.

    Leakage means continuation, so the evidence is a run of words in the prompt's
    own ORDER. Vocabulary overlap is not evidence: the prompt lists the commonest
    Hinglish words, so a genuine Hinglish sentence overlaps it heavily - an
    overlap-ratio test flags "aaj office meeting hai" and pastes nothing, silently.
    """
...
def _decode_wav(path: Path) -> np.ndarray:
    ...
    Keeps PyAV out of the build: handing faster-whisper a path makes it decode via
    PyAV, which bundles FFmpeg (62.6 MB, 12.9% of the install) to read a WAV we
    wrote moments earlier.

    Must match faster_whisper.audio.decode_audio - signed 16-bit mono at 16 kHz,
    divided by 32768.0 - so the default config yields the same array, bit for bit.
    The conversions below are the only divergence and only run on non-default
    configs: channels > 1 is averaged like FFmpeg's downmix, a non-16 kHz rate is
    linearly interpolated, cruder than FFmpeg's polyphase resampler. Both warn.
    """
    ...
    # int16 -> float32 in [-1, 1). Divisor is 32768.0, not 32767: matches
    # decode_audio and keeps bit-equality.
...
def _transcribe_local(audio_path: Path, cfg: dict) -> str:
    ...
    segments, info = model.transcribe(
        ...
        # A pinned language stops mis-detection on short clips turning the
        # dictation into a translation.
        language=cfg.get("language") or None,
        # Primes vocabulary and script; see config.py.
        initial_prompt=cfg.get("initial_prompt") or None,
        vad_filter=True,
        vad_parameters=dict(min_silence_duration_ms=500),
        condition_on_previous_text=False,
        # Timestamps stay on although nothing reads them. Whisper trains with
        # timestamp tokens interleaved, so suppressing them takes the decoder off
        # distribution: 68.9% -> 88.9% on Hindi, plus punctuation (corpus/ACCURACY.md).
        without_timestamps=False,
    )

    text = " ".join(seg.text.strip() for seg in segments).strip()
    lang = getattr(info, "language", "?")
    # Transcript text goes to DEBUG only: casper.log ships at INFO, is plaintext
    # beside the executable and is never rotated. INFO keeps only the event shape.
    log.info(
    ...
    if prompt and leaked_prompt(text, prompt):
        ...
        segments, info = model.transcribe(
            ...
            vad_parameters=dict(min_silence_duration_ms=500),
            condition_on_previous_text=False,
            # Same as the first pass, or the retry is not comparable to it.
            without_timestamps=False,
        )
        retry = " ".join(seg.text.strip() for seg in segments).strip()
        log.debug(f"Retry without prompt: {retry!r}")
        # Still matching means the audio had nothing usable; paste nothing.
...
# Seconds to upload and transcribe on the cloud backends. Separate from
# llm_timeout_seconds, which bounds the polish call at 20s - far too short to
# upload a five-minute WAV (max_record_seconds allows 300) on a home uplink.
